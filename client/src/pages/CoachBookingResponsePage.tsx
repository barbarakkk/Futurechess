...
export function CoachBookingResponsePage() {
  ...
  useEffect(() => {
    ...
    if (!id || !token || (action !== "accept" && action !== "decline")) {
    ...
    async function respond() {
      try {
        const response = await api.post(`/coach-bookings/${id}/respond`, { token, action });
        if (!mounted) {
          return;
        }
        if (response.data.alreadyResponded) {
          setResult({ kind: "alreadyResponded", status: response.data.booking.status });
        } else {
          setResult({ kind: response.data.booking.status === "confirmed" ? "confirmed" : "declined" });
        }
      } catch {
        if (mounted) {
          setResult({ kind: "invalid" });
  ...
  const content = (() => {
    switch (result.kind) {
      ...
      case "invalid":
        return { icon: <X className="h-6 w-6 text-red-600" aria-hidden />, title: t("response.invalidTitle"), body: t("response.invalidBody") };
      ...
      case "confirmed":
        return {
          ...
          title: t("response.confirmedTitle"),
          body: t("response.confirmedBody"),
        };
      case "declined":
        return {
          icon: <X className="h-6 w-6 text-muted-foreground" aria-hidden />,
          title: t("response.declinedTitle"),
          body: t("response.declinedBody"),
        };
    }
  })();
